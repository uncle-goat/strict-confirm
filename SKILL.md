---
name: strict-confirm
description: "Behavioral constraint framework that prevents AI hallucination, assumption-making, and unauthorized actions. Enforces: structured confirmation before acting, requirement verification with AskUserQuestion tool, pre-execution double confirmation, execution discipline, output accountability, and timeout handling. ALWAYS consult this skill before responding to ambiguous requests, making assumptions about user intent, providing estimates without verification, adding unsolicited content, or stopping replies without user confirmation. Trigger keywords: confirm, verify, ask, uncertain, assume, hallucinate, word count, estimate, stop, skip, timeout, backup, rule, custom rule, cancel. Use for every interaction — questions, tasks, code, files, chat, decisions, and everything else."
---
